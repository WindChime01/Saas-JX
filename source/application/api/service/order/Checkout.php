...
class Checkout
{
    ...
    private function setOrderGoodsGradeMoney()
    {
        ...
        foreach ($this->goodsList as &$goods) {
            ...
            if ($goods['is_alone_grade'] && isset($goods['alone_grade_equity'][$this->user['grade_id']])) {
                ...
                $discountRatio = helper::bcdiv($goods['alone_grade_equity'][$this->user['grade_id']], 10);  //百分比折扣
                }else{
                    $fix = $goods['alone_grade_equity'][$this->user['grade_id']];           //固定值折扣
                }
            } else {
                // 折扣比例
    ...
    private function setOrderCouponMoney($couponList, $couponId)
    {
        ...
        foreach ($this->goodsList as $key => &$goods) {
            $goods['coupon_money'] = helper::bcdiv($completed[$key]['coupon_money'], 100);
        }
        // 记录订单优惠券信息
        $this->orderData['coupon_id'] = $couponId;
        $this->orderData['coupon_money'] = helper::bcdiv($CouponMoney->getActualReducedMoney(), 100);
        return true;
    }
    ...
    public function createOrder($order)
    {
        ...
        $status = $this->model->transaction(function () use ($order) {
            // 创建订单事件
            // dump(createOrderEvent($order));die;
    ...
    private function createOrderEvent($order)
    {
        ...
        // 设置优惠券使用状态
        $order['coupon_id'] > 0 && UserCouponModel::setIsUse($order['coupon_id']);
        // 积分抵扣情况下扣除用户积分
