...
class Goods extends GoodsModel
{
    ...
    public function add(array $data)
    {
        ...
        if($data['alone_grade_type']==10){
        foreach ($data['alone_grade_equity'] as $val) {
            if($val>=10){
                $this->error = '折扣不能大于等于10';
                return false;
            }
        }
        }
        if($data['spec_type'] ==20 && $data['alone_grade_type'] ==20){
                $this->error = '多规格商品不允许设置固定金额';
                return false;
        }
        if($data['alone_grade_type']==20){
        foreach($data['alone_grade_equity'] as $key=>$val){
            // dump(ceil($val));die;
            if(ceil($val)==$val){
                $data['alone_grade_equity'][$key] =ceil($val).'.00' ;
                }
             }
        }
