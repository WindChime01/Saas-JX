...
class Order extends OrderModel
{
    ...
    public function confirmCancel($data)
    {
        ...
        $status = $this->transaction(function () use ($data) {
            ...
            $dealer_order = db::name('dealer_order')->where(['order_id'=>$this['order_id']])->find();
            if($dealer_order){
                db::name('dealer_order')->where(['order_id'=>$this['order_id']])->save(['is_invalid'=>1]);
            }
            // 更新订单状态
